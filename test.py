...
def inference(img, func, config, c, s):  # center scale
    """
    forward pass at 8HG time
    ...
    # TTA
    tmp1 = array2dict(func([inp]))  # {'det': (1, nstack, 16, 64, 64)}
    tmp2 = array2dict(func([inp[:, ::-1]]))

    tmp = {}  # {'det': (2, nstack, 16, 64, 64)}
    for ii in tmp1:
        tmp[ii] = np.concatenate((tmp1[ii], tmp2[ii]), axis=0)
    # tmp['det'][0, -1] (16, 64, 64)

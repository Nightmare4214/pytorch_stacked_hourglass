...
__config__ = {
    ...
    'inference': {
        ...
        'train_num_eval': 300,  ## number of train examples tested at 8HG time
    },
...
class Trainer(nn.Module):
    ...
    def __init__(self, model, inference_keys, calc_loss=None):
        ...
        self.model = model
        self.keys = inference_keys  # = 'imgs'
        self.calc_loss = calc_loss

    def forward(self, imgs, **inputs):  # imgs(B,256,256,3),inputs = {'heatmaps':(B,16,64,64) }
        inps = {}
        labels = {}
        ...
        if not self.training:
            ...
            combined_hm_preds = self.model(imgs, **inps)  # ([B, nstack, 16, 64, 64])
            if type(combined_hm_preds) != list and type(combined_hm_preds) != tuple:
                combined_hm_preds = [combined_hm_preds]
            loss = self.calc_loss(**labels, combined_hm_preds=combined_hm_preds)  # ([16, nstack])
...
def make_network(configs):
    ...
    def calc_loss(*args, **kwargs):
        return poseNet.calc_loss(*args, **kwargs)  # ([16, nstack])
    ...
    def make_train(batch_id, config, phase, **inputs):
        # inputs={'imgs':(B,256,256,3),'heatmaps':(B,16,64,64)}
        for i in inputs:
            try:
        ...
        if phase != 'inference':
            ...
            out = {}
            net = net.eval()
            result = net(**inputs)  # [(1,nstack,16,64,64)]
            if type(result) != list and type(result) != tuple:
                result = [result]
            out['preds'] = [make_output(i) for i in result]  # [(1,nstack,16,64,64)]
            return out
